...
export const Sidebar = (props) => {
  ...
  const handleClick = () => {
    dispatch(actions.cleanFilteredRecipes()); // Clear the filtered recipes when the "Clean filter" button is clicked
    dispatch(actions.setSearch("")); // Reset the search input
    dispatch(actions.setSeleccionadas([])); // Reset the selected diets
    dispatch(actions.cambiarPagina(1)); // Reset the current page
  };
